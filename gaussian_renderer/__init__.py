...
def render(viewpoint_camera: Camera, pc : GaussianModel, pipe, bg_color : torch.Tensor, scaling_modifier = 1.0, separate_sh = False, override_color = None, use_trained_exp=False):
    ...
    raster_settings = GaussianRasterizationSettings(
        ...
        projmatrix=viewpoint_camera.full_proj_transform,  # 注意这里是full版的 既有view也有proj 
    ...
    if override_color is None:
        if pipe.convert_SHs_python:
            shs_view = pc.get_features.transpose(1, 2).view(-1, 3, (pc.max_sh_degree+1)**2)  # (N, 3, 16)
    ...
    else:  # here, not separate sh to dc and rest 
        rendered_image, radii, depth_image = rasterizer(
            means3D = means3D,
            means2D = means2D,
            shs = shs,
